...
from memecoin_bot.broker import LiveBroker, PaperBroker, build_broker
...
def test_slippage_is_capped(settings):
    broker = PaperBroker(settings)
    fill = broker.buy(make_snapshot(liquidity_usd=100.0), 1_000_000.0)
    assert fill.price_usd / 0.001 <= 1.5


def test_empty_pool_is_penalised_not_crashed(settings):
    broker = PaperBroker(settings)
    fill = broker.buy(make_snapshot(liquidity_usd=0.0), 10.0)
    assert fill.quantity > 0
    assert fill.price_usd > 0.001
